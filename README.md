...
Профили `standard` и `deep` требуют накопленной истории PCP. История sysstat
необязательна: если корпоративная политика не позволяет включить его timers,
PCP остаётся основным источником ретроспективных данных. При установленном
sysstat его `pidstat`/`iostat`/`sar` дополнительно используются для активного
замера, а существующий архив — как дополнительный источник. Отсутствие или
неактуальность архива sysstat даёт предупреждение, но не блокирует аудит.
...
| `standard` | Рекомендуемый: всё основное, топ файлов/каталогов, размеры Docker volumes, история PCP и, при наличии, sysstat | 60 с |
...
gawk, grep, sed, jq, zip/unzip и lsof. Для `standard/deep` дополнительно
требуется PCP, для `deep` — smartmontools. `sysstat` необязателен; если он
установлен, используются активные samples и доступный локальный архив. При
шифровании требуется `age`.
