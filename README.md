...
Путь `~/server-doctor` используется как исполняемый launcher, поэтому каталог
с исходниками должен называться иначе, например `~/server-doctor-src`. Если
репозиторий уже скопирован в `~/server-doctor`, сначала переименуйте каталог:

```bash
mv ~/server-doctor ~/server-doctor-src
cd ~/server-doctor-src
make install-user
```
...
новой версии коллектор не останется от старой. Команду нужно выполнять без
`sudo`.

Если `make` ещё не установлен, одноразовый bootstrap можно выполнить напрямую:
